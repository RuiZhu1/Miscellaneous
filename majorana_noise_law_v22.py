#!/usr/bin/env python3
"""Majorana noise-kernel law v22 (slim rewrite of v21.4; CuPy-batched, falls back to NumPy).

Chain:  microscopic wire -> K(x) = d eps/d mu(x) -> N(xi_n) = K^T R(xi_n) K -> law.

Hypotheses tested (all falsifiable, all coefficients frozen from theory first):
...
Scan varies L, alpha, Ez, Delta, t0 AND mu (six parameters), at two operating phases:
...
def majorana(H, L):
    ...
    xs = np.arange(L, dtype=float)                    # site positions (v21.4 GPU bug: this was floor(i/4))
...
def make_figures(outdir, aud, arrs, pairs, xis):
    ...
    fig.tight_layout(); fig.savefig(os.path.join(outdir, "fig1_laws_v22.png"), dpi=200); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(os.path.join(outdir, "fig2_residuals_v22.png"), dpi=200); plt.close(fig)
...
def law_card(aud, xis):
    ...
    print("(10) design rule: minimise N_geom at fixed E_gap; detune the two wires by |Delta| >~ 2; at xi_n >> xi choose 'anti' (p0 small)")
    if "anti" in aud:
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Majorana noise-kernel law v22")
    ap.add_argument("--device", default="auto", choices=("auto", "cpu", "gpu")); ap.add_argument("--outdir", default="outputs_v22")
    ...
    print(f"Majorana noise-kernel law v22 | device = {dev}" + (f" ({cp.cuda.runtime.getDeviceProperties(0)['name'].decode()})" if dev == "gpu" else ""))
    t0 = time.time()
    ...
    with open(os.path.join(args.outdir, "points_v22.csv"), "w", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=list(flat[0].keys())); w.writeheader(); w.writerows(flat)
    np.savez_compressed(os.path.join(args.outdir, "kernels_v22.npz"), **{f"K_{i}_{r['phase']}": r["K"] for i, r in enumerate(recs)})
    with open(os.path.join(args.outdir, "results_v22.json"), "w") as fh:
        json.dump(clean({"args": vars(args), "device": dev, "audit": aud}), fh, indent=1)
    print(f"\noutputs -> {args.outdir}/  (results_v22.json, points_v22.csv, kernels_v22.npz, fig1_laws_v22.png, fig2_residuals_v22.png)")
